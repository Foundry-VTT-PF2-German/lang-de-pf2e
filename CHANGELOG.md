## v8.2.0.0
* Update auf Foundry 14.363
* Update auf PF2-Systemrelease 8.2.0
* Fertigstellung Übersetzung Sporenkrieg Bestiarium
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v8.1.2.0
* Update auf Foundry 14 und Babele 2.9.X
* Update auf PF2-Systemrelease 8.1.2
* Beginn Übersetzung Sporenkrieg Bestiarium
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v7.12.2.0
* Dies ist das finale Release für Foundry 13
* Update auf PF2-Systemrelease 7.12.2
* Einarbeitung der Paizo Spring Errata für Kernregeln: Spieler 2
* Übersetzung der Inhalte aus dem Ulisses Special "Monster aus Akiton"
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v7.11.3.1
* Funktionserweiterung für Lokalisierungsmodule zu Abenteuern
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v7.11.3.0
* Update auf PF2-Systemrelease 7.11.3
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v7.11.2.0
* Update auf PF2-Systemrelease 7.11.2
* Übersetzung Schlachtgetümmel (Bestiarium folgt noch)
* Pflege Dunkle Archive Remaster
* Pflege Paizo Spring Errata
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v7.9.1.0
* Update auf PF2-Systemrelease 7.9.1
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v7.8.0.1
* Ergänzung Übersetzung zum Modul Companion Compendia
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v7.8.0.0
* Update auf PF2 7.8.0
* Übersetzung Klauen des Tyrannen Bestiarium
* Ergänzung einer freien Übersetzung zum Web Supplement Dark Archives: The Lost Pages
* Ergänzung freier Übersetzung zu verschiedenen Web-Erweiterungen
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v7.7.4.0
* Update auf PF2 7.7.4
* Finalisierung Schatzhort Remaster
* Ergänzung freie Übersetzungen für Divine Mysteries Web Supplement
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v7.6.4.0
* Update auf PF2 7.6.4
* Übersetzung Der Pakt des Wildwaldes Bestiarium
* Fortführen Pflege Schatzhort Remaster
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v7.6.3.0
* Update auf PF2 7.6.3
* Übersetzung Flamme des Lebens inkl. Bestiarium
* Übersetzung Klippenwacht Bestiarium
* Fortführen Pflege Schatzhort Remaster
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v7.5.1.0
* Update auf PF2 7.5.1
* Fortführen Pflege Schatzhort Remaster
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v7.4.3.1
* Verschiedene visuelle Optimierungen (SC-Sheet, NSC, Sheet, Wurf-Dialog, ...)
* Fortführen Pflege Schatzhort Remaster
* Das Modul führt zukünftig auch die Lokalisierung verschiedener anderer Module. Aktuell enthalten sind: PF2e HUD, PF2e Dailies und PF2e Flatcheck Helper
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v7.4.3.0
* Update auf Foundry 13.348 und PF2 7.4.3
* Übersetzung Der Bund von Belkzen inkl. Bestiarium
* Fortführen Pflege Schatzhort Remaster
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v7.4.2.0
* Update auf Foundry 13.347 und PF2 7.4.2
* Fortführen Pflege Schatzhort Remaster
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v7.3.0.0
* Update auf Foundry 13.346 und PF2 7.3.0
* Pflege Änderungen Träume der Namenlosen Türme Remaster
* Fortführen Pflege Schatzhort Remaster
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v7.2.0.0
* Update auf Foundry 13.345 und PF2 7.2.0
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v7.1.1.1
* Mini-Fix zum Wechsel auf aktuelles Foundry-Coding
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v7.1.1.0
* Update auf Foundry 13 und PF2 7.1.1
* Die Änderungen aus dem Remaster des Schatzhorts sind enthalten, allerdings im ersten Schritt auf Englisch. Eine Übersetzung der Änderungen erfolgt sukzessive
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.12.4.0
* Da es noch einige weitere Releases des PF2-Systems gab, haben wir uns dazu entschieden noch ein weiteres Release für Foundry 12 zu veröffentlichen
* Fertigstellung Übersetzung zu Göttliche Geheimnisse
* Übersetzung Königsmacher Bestiarium
* Übersetzung Mythische Monster Bestiarium
* Übersetzung der Monster aus dem Ulisses Special "Monster aus Tian Xia"
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.11.1.1
* Dies ist das finale Relase für Foundry V12
* Fortsetzen Übersetzung zu Göttliche Geheimnisse
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.11.1.0
* Update auf PF2 6.11.1
* Übersetzung des Bestiariums zu Krieg der Unsterblichen
* Beginn Übersetzung zu Göttliche Geheimnisse
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.10.2.0
* Update auf PF2 6.10.2
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.9.0.2
* Einarbeitung Übersetzungen zu Krieg der Unsterblichen
* Übersetzung der Bestiarien zu Geteilte Wellen, Abendmahl in der Löwenloge, Im Zeichen der Roten Mantis, Little Trouble in Big Absalom, Für eine Handvoll Blumen und Für ein Paar Blumen mehr
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.9.0.1
* Fertigstellung Guns & Gears Remaster
* Übersetzung des Bestiariums zu Träume der Namenlosen Türme
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.9.0.0
* Update auf PF2-Systemversion 6.9.0
* Update auf Babele 2.6.1
* Einarbeitung Guns & Gears Remaster Teil 1
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.8.5.0
* Update auf PF2-Systemversion 6.8.5
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.8.4.1
* Kleiner Coding-Fix
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.8.4.0
* Update auf PF2-Systemversion 6.8.4
* Übersetzung des Bestiariums zu Zorn der Elemente
* Einarbeitung verschiedener gemeldeter Errata
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.8.1.1
* Übersetzung folgender Bestiarien: Tian Xia Kompendium, Monster aus Geb, Mehr Monster aus Geb, Noch Mehr Monster aus Geb, Die Krone des Koboldkönigs
* Einarbeitung der aktuellen Paizo Errata zu Zorn der Elemente
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.8.1.0
* Update auf PF2-Systemversion 6.8.1
* Fertigstellung Pflege Übersetzungen zu "Tian Xia"
* Einarbeitung der aktuellen Paizo Errata sowie weiterer gemeldeter Übersetzungsfehler
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.7.2.0
* Update auf PF2-Systemversion 6.7.2
* Ergänzung Übersetzungen zu den Bestiarien für: "Kompendium Schreckensgewölbe"
* Beginn Pflege Übersetzungen zu "Tian Xia"
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.7.1.0
* Update auf PF2-Systemversion 6.7.1
* Ergänzung Übersetzungen zu den Bestiarien für: "Kompendium Klippenwacht"
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.6.2.3
* Ergänzung Übersetzungen zu den Bestiarien für: "Die Gruft des Himmelskönigs", "Kompendium Gesetzlose von Alkenstern" und "Kompendium Zeit der Asche"
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.6.2.2
* Fertigstellung Übersetzungspflege "Kernregeln Spieler 2"
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.6.2.1
* Fix für die Übersetzung von Kompendium-Items in Akteuren
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.6.2.0
* Update auf PF2-Systemversion 6.6.2
* Zweiter Teil Übersetzungen zu "Kernregeln Spieler 2"
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.6.1.0
* Wir begrüßen LoxPaw als neuen Contributor für das DE-Modul
* Update auf PF2-Systemversion 6.6.1
* Überarbeitung SL-Schirm
* Ergänzung Übersetzungen zu "Ruf der Wildnis"
* Erster Teil Übersetzungen zu "Kernregeln Spieler 2"
* Ergänzung Übersetzungen zum Bestiarium für "Die Sieben Verhängnisse für Sandspitze"
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.4.1.0
* Update auf PF2-Systemversion 6.4.1
* Ergänzung Übersetzungen zu den Bestiarien für: "Die Nacht des Grauen Todes", "Dunkle Archive", "Das böse, alte Haus" und "Die Schlickpest"
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.4.0.0
* Update auf PF2-Systemversion 6.4.0
* In Abstimmung mit Ulisses werden wir zukünftig auch die Bestiarien zu Abenteuern, Abenteuerpfaden und Regelbüchern (ausgenommen reine Bestiarien-Bücher) in unserem DE-Modul übersetzen. Die folgenden Bestiarien sind ab dem aktuellen Release enthalten: Der Untergang von Peststein, Hochhelm, Der Reiseführer, Die Unfassbaren Lande, Schatten der Abenddämmerung, Der Teufelskreis der Feindschaft, Abenteuer in Otari
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.3.1.0
* Update auf PF2-Systemversion 6.3.1
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.2.3.0
* Update auf PF2-Systemversion 6.2.3
* Fertigstellung Einarbeitung Die Gruft des Himmelskönigs
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.2.2.0
* Update auf PF2-Systemversion 6.2.2
* Einarbeitung Errata zu Zahnräder & Zunderbüchsen sowie ZdvO: Das Mwangibecken
* Fortsetzung Einarbeitung Die Gruft des Himmelskönigs
* Fix zum Update von Akteursbildern unter Forge
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.2.1.1
* Minifix in Übersetzungscoding

## v6.2.1.0
* Update auf PF2-Systemversion 6.2.1
* Player Core 2 wurde ins PF2-System integriert. Bis zum Erscheinen der Deutschen Übersetzung sind sämtliche Inhalte hierzu auf Englisch verfügbar.
* Fortsetzung Einarbeitung Die Gruft des Himmelskönigs
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.1.3.0
* Freigabe für Foundry Release 12.330
* Beginn Einarbeitung Die Gruft des Himmelskönigs
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.0.4.2
* Freigabe für Foundry Release 12.328
* Einarbeitung Zeitalter der verlorenen Omen: Abstammungen & Herkünfte Errata
* Einarbeitung Zeitalter der verlorenen Omen: Weltenband Errata
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.0.4.1
* Fertigstellung Datenpflege Kernregeln: Spielleitung
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.0.4.0
* Update auf PF2-Systemversion 6.0.4
* Fortsetzung Datenpflege Kernregeln: Spielleitung
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.0.2.0
* Update auf PF2-Systemversion 6.0.2
* Fortsetzung Datenpflege Kernregeln: Spielleitung
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v6.0.1.0
* Update auf Foundry 12
* Update auf PF2-Systemversion 6.0.1
* Fortsetzung Datenpflege Kernregeln: Spielleitung
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v5.16.1.1
* Wichtig: Dies ist das letzte Release für PF2 5.X und damit für Founry 11
* Fertigstellung Datenpflege zu Kernregeln: Spieler
* Beginn Datenpflege Kernregeln: Spielleitung
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v5.16.1.0
* Update auf PF2-Systemrelease 5.16.1
* Fortsetzung Datenpflege zu Kernregeln: Spieler
* Kompatibilität zu Babele 2.5.5
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v5.16.0.0
* Update auf PF2-Systemrelease 5.16.0
* Neues übersetztes Kompendium: Zeit der Asche Bestiarium
* Fortsetzung Datenpflege zu Kernregeln: Spieler
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v5.15.3.1
* Fix zum Mwangibecken-Bestiarium
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v5.15.3.0
* Update auf PF2-Systemversion 5.15.3
* Übersetzung Kernregeln: Spieler - Teil 1
* Übersetzung Bestiarium zu "Das Buch der Toten"
* Übersetzung Bestiarium zu "Das Mwangibecken"
* Verschiedene Formatierungskorrekturen
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v5.15.1.0
* Wir begrüßen Paktai als neuen Contributor für das DE-Modul
* Update auf PF2-Systemversion 5.15.1
* Übersetzung Inhalte zum Abenteuerpfad "Träume der Namenlosen Türme"
* Neuformatierung Gottheiten
* Verschiedene Formatierungskorrekturen
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v5.15.0.2
* Übersetzung Inhalte zum Abenteuerpfad "Königsmacher"
* Verschiedene Formatierungskorrekturen
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v5.15.0.1
* Technisches Update zur Übersetzung von Tokens in Szenen
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v5.15.0.0
* Update auf PF2-Systemversion 5.15.0
* Übersetzung Inhalte zum Abenteuer "Ins Schwarze!"
* Beginn Übersetzung Inhalte zum Abenteuerpfad "Königsmacher"
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v5.14.4.1
* Übersetzung Inhalte zum Abenteuer "Geteilte Wellen"
* Übersetzung Inhalte zum Abenteuer "Im Zeichen der Roten Mantis"
* Übersetzung Inhalte zum Abenteuer "Little Trouble in Big Absalom"
* Übersetzung Inhalte zum Abenteuer "Teufelskreis der Feindschaft"
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v5.14.4.0
* Update auf PF2-Systemversion 5.14.4
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v5.14.3.0
* Update auf PF2-Systemversion 5.14.3
* Implementierung Support für Automated Animations. Übersetzte Einträge sollten jetzt korrekte Animationen anzeigen. Danke an n1xx1 für den Coding-Vorschlag
* Update des "Bedrohung unter Otari" Bestiariums auf Remaster
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v5.13.6.1
* Verschiedene Coding-Optimierungen
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v5.13.6.0
* Update auf PF2-Systemversion 5.13.6
* Verschiedene Formatierungskorrekturen
* Neues übersetztes Kompendium: Schreckensgewölbe Bestiarium
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v5.13.5.0
* Update auf PF2-Systemversion 5.13.5
* Verschiedene Formatierungskorrekturen
* Neues übersetztes Buch: Rusthenge
* Neues übersetztes Kompendium: Rusthenge Bestiarium
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v5.12.7.0
* Update auf PF2-Systemversion 5.12.7
* Überarbeitung Archetyp-Journal
* Verschiedene Formatierungskorrekturen
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v5.12.5.0
* Update auf PF2-Systemversion 5.12.5
* Überarbeitung Archetyp-Journal
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v5.11.5.1
* Neues übersetztes Buch: Zorn der Elemente
* Neues übersetztes Kompendium: Bedrohung unter Otari Bestiarium
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v5.11.5.0
* Update auf PF2-Version 5.11.5
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v5.11.4.0
* Update auf PF2-Version 5.11.4
* WICHTIG: Die Version 5.11.4.0 enthält die umfangreichen Regeländerungen der Bücher Player Core und GM Core.
* Bei Spielelementen, die umfangreiche Textänderungen erfahren haben, wurde die Deutsche Übersetzung entfernt. Diese wird nachgepflegt, wenn die Bücher auf Deutsch veröffentlicht wurden.
* Eine ausführliche Übersicht wesentlicher Änderungen im Spielsystem findet ihr im PF2-Journal "Remaster-Änderungen"
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne als Issue auf unserer GitHub-Projektseite.

## v5.8.3.0
* Update auf Foundry-Version 11.315 und PF2-Version 5.8.3
* WICHTIG: Die PF2-Version 5.8.3 ist die letzte Systemversion, bevor die umfangreichen Änderungen durch Pathfinder Player Core und Pathfinder GM Core Einzug ins System halten.
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne hier: https://github.com/Foundry-VTT-PF2-German/lang-de-pf2e/issues

## v5.8.2.0
* Update auf PF2-Version 5.8.2
* Neues übersetztes Buch: Hochhelm
* Neues übersetztes Kompendium: Gesetzlose von Alkenstern Bestiarium
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne hier: https://github.com/Foundry-VTT-PF2-German/lang-de-pf2e/issues

## v5.7.4.0
* Update auf Foundry-Version 11.313 und PF2-Version 5.7.4
* Neues übersetzte Journal-Einträge: Abstammungen und Klassen
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne hier: https://github.com/Foundry-VTT-PF2-German/lang-de-pf2e/issues

## v5.6.2.0
* Update auf Foundry-Version 11.311 und PF2-Version 5.6.2
* Überarbeitung alchemistische Bomben und Elixiere des Grundregelwerks
* Remaster-Update: Negativer und Positiver Energieschaden wurden geändert in Leerenschaden uns Lebensschaden
* Neues übersetztes Kompendium: NSC-Galerie
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne hier: https://github.com/Foundry-VTT-PF2-German/lang-de-pf2e/issues

## v5.5.3.1
* Update auf Foundry-Version 11.309
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne hier: https://github.com/Foundry-VTT-PF2-German/lang-de-pf2e/issues

## v5.5.3.0
* Update auf PF2-Systemrelease 5.5.3
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne hier: https://github.com/Foundry-VTT-PF2-German/lang-de-pf2e/issues

## v5.5.2.0
* Update auf PF2-Systemrelease 5.5.2
* Verschiedene Übersetzungskorrekturen
* Übersetzung der Inhalte zum Quellenbuch "Heißsporne"
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne hier: https://github.com/Foundry-VTT-PF2-German/lang-de-pf2e/issues

## v5.4.3.0
* Fortführung Einarbeitung Pathfinder Remastered
* Fortsetzung Überarbeitung der Archetyp-Journaleinträge: Assassine der roten Mantis, Magischer Krieger, Untotenjäger, Zaubertrickser
* Verschiedene Übersetzungskorrekturen
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne hier: https://github.com/Foundry-VTT-PF2-German/lang-de-pf2e/issues

## v5.3.2.0
* HINWEIS: Dieses Release setzt auf der PF2-Version 5.3.2 auf, die erste Remastered-Inhalte umsetzt. Informationen hierzu findet ihr auf unserer Modulseite: https://foundryvtt.com/packages/lang-de-pf2e
* Umfangreicher Wechsel auf @Damage-Formatierung für Inline-Schadenswürfe
* Beginn Einarbeitung Pathfinder Remastered
* Erste Überarbeitung der Archetyp-Journaleinträge: Lebender Monolith, Mumie, Ringer, Zauberschütze
* Verschiedene Übersetzungskorrekturen
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne hier: https://github.com/Foundry-VTT-PF2-German/lang-de-pf2e/issues

## v5.2.3.1
* HINWEIS: Die Version 5.2.3.1 ist die letzte im aktuellen Regelwerk, das PF2-System beginnt ab Version 5.3 mit der Einarbeitung der Remastered-Regeln, unsere nächste Modulversion setzt darauf auf
* Verschiedene Übersetzungskorrekturen
* Übersetzung der Inhalte zum Regelbuch "Schatzhort"
* Solltet ihr veraltete Einträge, falsche Formatierungen oder ähnliche Fehler finden, dann meldet sie gerne hier: https://github.com/Foundry-VTT-PF2-German/lang-de-pf2e/issues

## v5.2.3.0
* HINWEIS: Die Versionen 5.2.3.x sind die letzten im aktuellen Regelwerk, das PF2-System beginnt ab Version 5.3 mit der Einarbeitung der Remastered-Regeln
* Wir begrüßen Vendergast als neuen Contributor für das DE-Modul
* Update für PF2-Version 5.2.3
* Verschiedene Übersetzungskorrekturen
* Übersetzung der Inhalte zu "Die Gesetzlosen von Alkenstern" - Abenteuerpfad, Spieler-Leitfaden, Kompendium
* Übersetzung der Inhalte zu den Abenteuern "Für eine Handvoll Blumen" und "Hort des Wissens"

## v5.1.2.0
* Update für PF2-Version 5.1.2
* Verschiedene Übersetzungskorrekturen
* Einbauen neues Basis-Coding für zukünftige Archetyp-Übersetzung

## v5.1.0.0
* Update für PF2-Version 5.1.0
* Übersetzung Inhalte zum Abenteuer "Die Krone des Koboldkönigs"
* Korrekturen Sonderzeichen

## v5.0.2.1
* Übersetzung Texte für dynamische Effekte, z.B. Schutzaura

## v5.0.2.0
* Initialrelease für Foundry V11 und PF2 V5

## v4.12.9.0
* Finaler Release für PF2 Version 4 unter Foundry 10

## v4.12.3.2
* Korrektur Download URL

## v4.12.3.1
* Korrektur Manifest URL

## v4.12.3.0
* Anpassungen für PF2e v4.12.3
* Ergänzung Übersetzungen für Die Unfassbaren Lande

## v4.11.2.0
* Anpassungen für PF2e v4.11.2
* Ergänzung Übersetzungen für Die Dunklen Archive

## v4.10.0.0
* Anpassungen für PF2e v4.10.0
* Umfassende Übersetzungen für Regelelement-Texte

## v4.10.3.0
* Anpassungen für PF2e v4.10.3
* Aufräumen Alt-Coding
* Ergänzen Coding um Programmfehler bei Datenformatänderungen im PF2-System zu verhindern (Danke an n1xx1 der Italienischen Lokalisierung für die Bereitstellung des Codings)
* Umfassende Übersetzungen für Talent- und Zaubereffekte

## v4.10.4.0
* Anpassungen für PF2e v4.10.4
* Umfassende Übersetzungen für Talent-Effekte
